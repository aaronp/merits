...
import { computeArgsHash, sha256Hex, signPayload } from '../core/crypto';
...
import { createMessageRouter, type MessageRouter } from '../core/runtime/router';
...
export class MessageBusClient {
  ...
  async send(
    ...
    // Issue challenge
    const { challengeId, payload } = await this.issueChallenge(credentials.aid, 'send', argsHash);
   ...
   */
  async receive(recpAid: string, credentials: AuthCredentials): Promise<Message[]> {
    // Compute args hash - binds to recpAid only
    const argsHash = await this.computeArgsHash({ recpAid });

    // Issue challenge
    const { challengeId, payload } = await this.issueChallenge(credentials.aid, 'receive', argsHash);
   ...
   */
  subscribe(_recipientDid: string, _callback: (messages: Message[]) => void): () => void {
  ...
  async acknowledge(
    ...
    // Issue challenge
    const { challengeId, payload } = await this.issueChallenge(credentials.aid, 'ack', argsHash);
   ...
   */
  private async signPayload(payload: any, privateKey: Uint8Array, keyIndex: number): Promise<string[]> {
  ...
  async createAuth(
    ...
    const { challengeId, payload } = await this.issueChallenge(credentials.aid, purpose, argsHash);
    const sigs = await this.signPayload(payload, credentials.privateKey, 0);
