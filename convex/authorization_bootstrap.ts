...
export const bootstrapOnboarding = mutation({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      if (existingRoles || existingUserRoles) {
        ...
        if (onboardingGroup && args.adminAid) {
          const existingMembership = await ctx.db
            .query('groupMembers')
            .withIndex('by_group_aid', (q: any) => q.eq('groupChatId', onboardingGroup._id).eq('aid', args.adminAid))
            .first();
          ...
          if (!existingMembership) {
            ...
            const allMessages = await ctx.db
              ...
              .collect();
            const currentSeqNo = allMessages.length > 0 ? Math.max(...allMessages.map((m: any) => m.seqNo)) : -1;
        ...
        if (existingAdmin) {
          ...
          if (messageUsersPermission) {
            const hasMapping = existingAdminRP.some((rp: any) => rp.permissionId === messageUsersPermission._id);
            if (!hasMapping) {
              await ctx.db.insert('rolePermissions', {
        ...
        if (userRole) {
          ...
          if (messageUsersPermission) {
            const hasMapping = existingUserRP.some((rp: any) => rp.permissionId === messageUsersPermission?._id);
            if (!hasMapping) {
              await ctx.db.insert('rolePermissions', {
                roleId: userRole?._id,
                permissionId: messageUsersPermission?._id,
          ...
          if (createGroupsPermission) {
            const hasMapping = existingUserRP.some((rp: any) => rp.permissionId === createGroupsPermission?._id);
            if (!hasMapping) {
              await ctx.db.insert('rolePermissions', {
                roleId: userRole?._id,
                permissionId: createGroupsPermission?._id,
          ...
          // Ensure user role has permission to read groups (members can read their groups)
          const readGroupsPermKey = PERMISSIONS.CAN_READ_GROUPS;
          let readGroupsPermission = await ctx.db
            .query('permissions')
            .withIndex('by_key', (q: any) => q.eq('key', readGroupsPermKey))
            .first();

          if (!readGroupsPermission) {
            const pid = await ctx.db.insert('permissions', {
              key: readGroupsPermKey,
              data: undefined, // No restrictions - users with 'user' role can read groups they're members of
              adminAID: 'SYSTEM',
              actionSAID: 'bootstrap/perms',
              timestamp: now,
            });
            readGroupsPermission = await ctx.db.get(pid);
            console.log('Bootstrap: Created CAN_READ_GROUPS permission');
          }

          if (readGroupsPermission) {
            const hasMapping = existingUserRP.some((rp: any) => rp.permissionId === readGroupsPermission?._id);
            if (!hasMapping) {
              await ctx.db.insert('rolePermissions', {
                roleId: userRole?._id,
                permissionId: readGroupsPermission?._id,
                adminAID: 'SYSTEM',
                actionSAID: 'bootstrap/map',
                timestamp: now,
              });
              console.log('Bootstrap: Linked user role to CAN_READ_GROUPS permission');
            }
          }
        }
      ...
      const hasMessageUsersMapping = existingAdminRP.some((rp: any) => rp.permissionId === messageUsersPermission?._id);
      ...
      const hasCreateGroupsMapping = existingAdminRP.some((rp: any) => rp.permissionId === createGroupsPermission?._id);
      ...
      const hasAssignRolesMapping = existingAdminRP.some((rp: any) => rp.permissionId === assignRolesPermission?._id);
