...
export interface MeritsVault {
  ...
  /**
   * Update identity metadata without touching the private key
   *
   * This is more efficient and secure than re-storing the entire identity.
   * Used for updating registration status, rotation timestamps, etc.
   *
   * @param name - Identity name
   * @param patch - Partial metadata to merge with existing metadata
   * @throws Error if identity not found
   */
  updateMetadata(name: string, patch: Partial<Record<string, any>>): Promise<void>;

  /**
   * Get public key from metadata (no private key access)
   *
   * Returns the public key that was stored in metadata at identity creation.
   * This avoids the need to export the private key just to derive the public key.
   *
   * @param name - Identity name
   * @returns Public key bytes
   * @throws Error if identity not found or public key not in metadata
   */
