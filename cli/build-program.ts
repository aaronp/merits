...
import { accessAllow, accessClear, accessDeny, accessList, accessRemove } from './commands/access';
...
import { addGroupMember, createGroup, groupInfo, leaveGroup, listGroups, removeGroupMember } from './commands/group';
...
import { permissionsCreate, rolesAddPermission, rolesCreate, usersGrantRole } from './commands/rbac';
...
Documentation:
  ...
  program.hook('preAction', (_thisCommand, actionCommand) => {
    ...
    const client = credentials ? createMeritsClientFromCredentials(credentials, config) : null; // Commands without credentials will need to handle this
...
Examples:
  ...
  program.command('whoami').description('Display current session information').action(whoami);
...
Output:
  ...
  groupCmd.command('remove <group-id> <member-aid>').description('Remove member from group').action(removeGroupMember);

  groupCmd.command('leave <group-id>').description('Leave a group').action(leaveGroup);
...
Examples:
  ...
  const rbacUsersCmd = rbacCmd.command('users').description('Manage user roles');
